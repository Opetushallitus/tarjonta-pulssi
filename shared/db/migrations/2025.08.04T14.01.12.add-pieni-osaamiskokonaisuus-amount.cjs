const up = async ({ context: pg }) => {
  await pg.query(`
 ALTER TABLE toteutus_amounts ADD COLUMN IF NOT EXISTS pieni_osaamiskokonaisuus_amount bigint default 0; 
  `);
  await pg.query(`
 ALTER TABLE toteutus_amounts_history ADD COLUMN IF NOT EXISTS pieni_osaamiskokonaisuus_amount bigint default 0; 
   `);
  ...
  begin
    insert into toteutus_amounts_history (
      ...
      system_time
        ...
        old.pieni_osaamiskokonaisuus_amount,
        old.tila,
        old.amount,
        old.transaction_id,
